#include <ThingsCloudMQTT.h>

//======================================================
// 设置 ssid / password，连接到你的 WiFi AP
const char *ssid = "30";
const char *password = "xl870827";
String zt = "on";

// 在 ThingsCloud 控制台的设备详情页中，复制以下设备连接信息
// https://console.thingscloud.xyz
#define THINGSCLOUD_MQTT_HOST "sh-1-mqtt.iot-api.com"
#define THINGSCLOUD_DEVICE_ACCESS_TOKEN "flzomzqd5hcaq5sz"
#define THINGSCLOUD_PROJECT_KEY "Vp5wkziJPa"
//======================================================

ThingsCloudMQTT client(
    THINGSCLOUD_MQTT_HOST,
    THINGSCLOUD_DEVICE_ACCESS_TOKEN,
    THINGSCLOUD_PROJECT_KEY);

// 上报数据的间隔时间计时器
unsigned long timer1 = millis();
// 设置定时上报数据的时间间隔，单位是 ms。免费版项目请务必大于30秒，否则设备可能会被限连。
const int report_interval = 1000 * 35;
int flag = 0;

void setup()
{
  Serial.begin(115200);
  pinMode(4, INPUT);
  // 允许 SDK 的日志输出
  client.enableDebuggingMessages();

  // 连接 WiFi AP
  client.setWifiCredentials(ssid, password);
}

// 读取并发布传感器数据到 ThingsCloud
void pubSensors()
{
  // 生成属性 JSON
  DynamicJsonDocument obj(512);
  obj["bj"] = zt;

  char attributes[512];
  serializeJson(obj, attributes);
  // 调用属性上报方法
  client.reportAttributes(attributes);
}

// 必须实现这个回调函数，当 MQTT 连接成功后执行该函数。
void onMQTTConnect()
{
  // 延迟 5 秒上报首次传感器数据
  client.executeDelayed(1000 * 5, []()
                        { pubSensors(); });
}
int count = 0;
int pinState = 0;
void loop()
{
  client.loop();
  // Serial.println(flag);
  // 串口获得数据
  pinState = digitalRead(4);
  // Serial.print("pinstate:");
  // Serial.println(pinState);
  // Serial.print("count:");
  // Serial.println(count);
  if (pinState == 1 && count == 0)
  {
    count = count + 1;
    flag = 1;
    Serial.println("接收到擦碰信息");
  }
  if (flag == 1 && count == 1)
  {
    // 按间隔时间上报传感器数据
    if (millis() - timer1 > report_interval)
    {
      Serial.print("flag: ");
      Serial.println(flag);
      timer1 = millis();
      pubSensors();
      flag = 0;
      count = 0;
    }
  }
  vTaskDelay(300 / portTICK_PERIOD_MS); // 1秒发送一次
}