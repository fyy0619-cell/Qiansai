...
void initiator()
{
    if (!wait_ack && !wait_final && (counter == 0))
    {
        ...
        tag_tx_msg[MSG_FUNC_IDX] = FUNC_CODE_INTER;
    ...
    { /* received all final msg */
        ...
        for (int i = 0; i < counter; i++)
        {
            ...
            tof = ((t_round_1 * t_round_2 - t_reply_1 * t_reply_2) /
                   ...
                   33) *
                  DWT_TIME_UNITS; // 34
            distance = tof * SPEED_OF_LIGHT - 0.55;
...
void responder()
{
    ...
    if (wait_poll && (counter == POLL_NUM))
    { /* received all poll */
        /* send poll to U4 - U6, ack to U1, U2 */
        tx_time = (poll_rx_from_others_ts[counter - 1] + (RX_TO_TX_DLY_UUS * UUS_TO_DWT_TIME)) >> 8;
        tx_ts = (((uint64_t)(tx_time & 0xFFFFFFFEUL)) << 8) + TX_ANT_DLY;
        dwt_setdelayedtrxtime(tx_time);
